...
for line in $(find . -name 'gradlew'); do
   ...
   echo "y" | sudo /opt/google-cloud-sdk/bin/gcloud firebase test android run --app app/build/outputs/apk/app-debug.apk --test app/build/outputs/apk/app-debug-androidTest.apk -d Nexus5X -v 21,25 -l fr --results-bucket cloud-test-android-devrel-ci
   code=${PIPESTATUS[0]}
   if [ "$code" -ne "0" ]; then
       exit $code
   fi
